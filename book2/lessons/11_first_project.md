...
### 11.2.1 단계 ① 작업 폴더에서 Claude Code를 켭니다

6차시에서 만들었던 그 작업 폴더로 갑니다. 파워셸을 열고 아래처럼 입력합니다.

!!! quote "🗣️ 이대로 입력해 확인합니다"
    ```
...
### 11.2.2 단계 ② 첫 부탁을 건넵니다 — "리듬게임 만들어줘"
...
!!! warning "⚠️ 조심 — 중간에 창을 닫지 마십시오"
    파일을 만드는 도중에 터미널 창을 닫거나 컴퓨터를 재우면 작업이 중간에 멈춥니다. "완료했습니다" 또는 다음 부탁을 받을 준비가 된 표시가 나올 때까지 창을 그대로 둡니다.

### 11.2.4 단계 ④ 게임을 브라우저에서 켭니다

AI가 파일 생성을 완료하면, 이제 실제로 게임을 켜 볼 시간입니다.

!!! warning "⚠️ 조심 — 대화창을 먼저 빠져나와야 합니다"
    지금 터미널은 Claude Code와 대화하는 창(`claude`) 안입니다. 이 상태에서 아래 명령어를 그냥 치면 AI가 자기한테 하는 부탁으로 오해합니다. 대화창에 **`/exit`**를 입력하고 엔터를 눌러 먼저 파워셸 상태(맨 앞 `PS`가 보이는 상태)로 빠져나오세요.

!!! quote "🗣️ 이대로 입력해 확인합니다 (대화방 빠져나오기)"
    ```
    /exit
    ```

그다음, 파워셸 창에 아래 명령을 입력합니다.

!!! quote "🗣️ 이대로 입력해 확인합니다 (게임 켜기)"
    ```
    npx expo start --web
    ```
...
잠시 뒤 **인터넷 창(브라우저)이 저절로 열립니다.** 혹시 안 열리면, 브라우저를 직접 열고 주소창에 `localhost:8081`을 입력해 확인합니다.

### 11.2.5 단계 ⑤ 첫 화면을 확인합니다
...
!!! tip "💡 이럴 땐 — 터미널이 게임 서버로 '먹통'처럼 보여요"
    `npx expo start --web`을 실행한 창은 게임을 계속 돌리는 데 쓰느라, 글자를 입력해도 반응이 없어 보입니다. 정상입니다. **그 창은 그대로 두고**, Claude Code에게 계속 말을 걸고 싶으면 **새 터미널 창을 하나 더 엽니다**(3차시 방법 그대로). 12차시에서 이 방법을 바로 씁니다.

!!! warning "⚠️ 조심 — 실행 폴더를 꼭 확인하세요"
    `npx expo start --web`은 **지금 터미널이 서 있는 폴더**를 기준으로 동작합니다. 엉뚱한 폴더(예: 바탕화면 맨 위)에서 실행하면 오류가 납니다. 항상 `cd $HOME\Desktop\rhythm-game`으로 먼저 이동했는지 확인합니다.
