# CHAPTER 6 · Claude Code 설치와 첫 대화
...
!!! warning "⚠️ 조심 — 먼저 유료 구독이 필요합니다"
    Claude Code는 **무료 계정으로는 쓸 수 없고**, 최소 **Pro 구독**(대략 월 3만원대)이 있어야 합니다. 요금은 자주 바뀌니 정확한 금액은 공식 사이트(`claude.com`)에서 확인합니다. 아직 구독이 없다면 먼저 가입해 두고 오세요.

---

## 6.2 따라 하기

### 6.2.1 단계 ① 설치 명령을 붙여넣습니다

파워셸(3차시에서 연 그 검은 창)을 엽니다. 맨 앞에 **`PS`**가 있는지 다시 확인합니다. 그리고 아래 한 줄을 **그대로 복사해 붙여넣고** 엔터를 누릅니다.
...
!!! warning "⚠️ 조심 — `irm이 인식되지 않습니다`가 뜨면"
    파워셸이 아니라 옛날 창(CMD)에 붙여넣은 것입니다. 창을 닫고 3차시대로 **[터미널/Windows PowerShell]**을 다시 엽니다(맨 앞 `PS` 확인).

### 6.2.2 단계 ② 작업 폴더에서 Claude Code를 켭니다
...
연결이 끝났으면, 제대로 깔렸는지 버전을 확인합니다. (터미널에서 Claude Code를 잠깐 빠져나오려면 `/exit` 입력.)
...
### 6.2.5 단계 ⑤ AI에게 첫 부탁을 건넵니다

이제 진짜입니다. 터미널에 다시 **`claude`**를 입력하고 엔터를 눌러 대화창을 켭니다.
...
> **다음 차시 예고** — 다음 7차시에서는 Claude Code를 내 입맛에 맞게 길들이는 법(**CLAUDE.md** 메모 작성 및 슬래시 명령)을 배웁니다.
